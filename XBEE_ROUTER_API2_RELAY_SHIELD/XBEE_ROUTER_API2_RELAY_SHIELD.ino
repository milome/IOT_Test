...
#define WATER_LEVEL_THRESHOLD 615 //mapping to the sensor value 4.1 cm
#define WATER_FLOW_THRESHOLD 100 //Avoid too long time noise, normally flow will keep on a high level > 30
...
int hallsensor = 3; //定义hallsensor 为数字口3
...
// payload[4]-[5]:  hall sensor(water flow) - D3(Need interruption support D2 OR D3!)
...
int statusLed = 13;
int errorLed = 13;
/* Following for pins are reserved by relay shield */
//Test h
int relayMotorPin = 2;
int relayAcidInjectPin = 7;
int relayNo4Pin = 8;
int relayFixPin = 10;
/* End of relay pins */
...
void setup() {
  ...
  pinMode(relayAcidInjectPin, OUTPUT);
  pinMode(relayNo4Pin, OUTPUT);
  pinMode(relayFixPin, OUTPUT);
  //Default enable the Motor
  digitalWrite(relayMotorPin, HIGH);
  digitalWrite(relayAcidInjectPin, LOW);
  digitalWrite(relayNo4Pin, LOW);
  digitalWrite(relayFixPin, LOW);
   ...
   * int.1 D3
   */
  attachInterrupt(1, rpm, RISING); //定义中断进程 http://www.arduino.cn/thread-2421-1-1.html
...
void readPacketIn() {
  ...
  if (xbee.getResponse().getApiId() == ZB_RX_RESPONSE) {
    ...
    if (rx.getOption() == ZB_PACKET_ACKNOWLEDGED) {
      ...
      if(rx.getData(0) == 'd' && rx.getData(1) == 'o' && rx.getData(2) == 'w' && rx.getData(3) == 'n' && rx.getData(4) == '4'){
         turnOnRelay(relayNo4Pin);
      } else if(rx.getData(0) == 'f' && rx.getData(1) == 'i' && rx.getData(2) == 'x') {
        // when to stop?
         turnOnRelay(relayFixPin);
      }  else if(rx.getData(0) == 'n' && rx.getData(1) == 'o' && rx.getData(2) == 'f' && rx.getData(3) == 'i' && rx.getData(4) == 'x') {
        // when to stop?
         turnOffRelay(relayFixPin);
      }else if(rx.getData(0) == 'u' && rx.getData(1) == 'p' && rx.getData(2) == '4') {
         turnOffRelay(relayNo4Pin);
      } else if(rx.getData(0) == 'i' && rx.getData(1) == 'n' && rx.getData(2) == 'j' && rx.getData(3) == 'e' && rx.getData(4) == 'c' && rx.getData(5) == 't') {
         turnOnRelay(relayAcidInjectPin);
//         playNotes(3);
      } else if(rx.getData(0) == 's' && rx.getData(1) == 't' && rx.getData(2) == 'o' && rx.getData(3) == 'p' && rx.getData(4) == 'i' && rx.getData(5) == 'n') {
         turnOffRelay(relayAcidInjectPin);
//         playNotes(3);
      }
    } 
    else {
...
void loop() {   
  ...
  // When water flow < 2L/H, just turn of the pump for now.
  if(pinHall < WATER_FLOW_THRESHOLD && pinWaterLevel < WATER_LEVEL_THRESHOLD) {
    turnOffRelay(relayMotorPin);
  } else {
    turnOnRelay(relayMotorPin);
  }
  
  //Turn off the fix valve if it's opened
  if(pinPH > 699){
    turnOffRelay(relayFixPin);
  }
